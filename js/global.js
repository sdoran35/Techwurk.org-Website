...
jq(document).ready( function() {
		...
		checkboxes.each( function(i) {
			if(jq(this).is(':checked')) {
				if ( jq('#m-' + jq(this).attr('value')).hasClass(currentClass) ) {
					...
					var thread_count = jq('#m-' + jq(this).attr('value') + ' span.unread-count').html();

					jq('#m-' + jq(this).attr('value') + ' span.unread-count').html(unreadCount);
					jq('#m-' + jq(this).attr('value') + ' span.unread-count').css('display', unreadCountDisplay);
	...
	jq("select#message-type-select").change(
		function() {
			...
			switch(selection) {
				case 'unread':
					var checkboxes = jq(".unread input[type='checkbox']");
				break;
				case 'read':
					var checkboxes = jq(".read input[type='checkbox']");
				break;
			}
			if ( selection != '' ) {
				checkboxes.each( function(i) {
